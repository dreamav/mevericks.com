<?php
/*
Plugin Name: IshYoBoy Shortcodes
	...
	class Ishyoboy_Shortcodes extends Ish_Plugin_Base {
		...
		public function action_admin_scripts_init() {
			...
			wp_localize_script( 'ish-custom-views', 'iyb_pagebuilder', Array(
				'main_button_title' => __( 'Switch to Page Builder', 'ish_sc_plugin' ),
				'main_button_title_revert' => __( 'Switch to WordPress Editor', 'ish_sc_plugin' ),
				...
				'trans_strings' => Array(
					'accordion' => __( 'Accordion', 'ish_sc_plugin' ),
					'toggle' => __( 'Toggle', 'ish_sc_plugin' ),
				)
			) );
		}
		...
		public function shortcode_callback( $atts, $content = null, $tag ){
			...
			if ( file_exists($template) ) {
				...
				trigger_error(sprintf( __( 'Wrong template for `%s` shortcode. Please make sure the template exists.', 'ish_sc_plugin' ), $tag) );
			}
		...
		public function vc_theme_colors_css() {
			...
			for ($i = 1; $i <= IYB_COLORS_COUNT; $i++ ){
				...
				// Row
				echo '.wpb_row_container[data-ish-color="color' . $i . '"] > .wpb_vc_column { background: ' . $ish_options['color' . $i] . ";}\n";
		...
		public function load_vc_config() {
			...
			if ( file_exists( $config ) ) {
				...
				trigger_error( __( 'Missing config file. Please make sure the file exists.', 'ish_sc_plugin' ) );
		...
		public function get_theme_colors_array() {
			...
			if ( ! empty( $ish_options ) ){
				...
				for ($i = 1; $i <= IYB_COLORS_COUNT; $i++ ){
					...
					if ( isset( $ish_options['color' . $i ] ) ){
						//$ish_theme_colors[ sprintf( __( 'Color %d', 'ish_sc_plugin' ), $i )  ] = 'color' . $i  ;
			...
			//$ish_theme_colors[ __( 'Advanced', 'ish_sc_plugin' ) ] = 'advanced';
		...
		public function get_available_taxonomy_terms( $taxonomy = 'category' ) {
			...
			if ( ! empty( $categories ) ){
				...
				foreach ( $categories as $category ){
					$return[ __( $category->name, 'ish_sc_plugin' ) ] = $category->slug;
				}
		...
		public function remove_default_vc_shortcodes(){
			...
			if ( ! isset( $ish_options ) || ! isset( $ish_options['plugin_sc_enable_vc_shortcodes'] ) || ( '0' == $ish_options['plugin_sc_enable_vc_shortcodes'] ) ){
				if ( function_exists('vc_remove_element') ){
					...
					$vc_posts_grid_instance = WPBakeryVisualComposer::getInstance()->getShortCode('vc_posts_grid');
					remove_action( 'admin_init', array( $vc_posts_grid_instance, 'jsComposerEditPage' ), 6 );
					vc_remove_element('vc_posts_grid');

					$vc_carousel_instance = WPBakeryVisualComposer::getInstance()->getShortCode('vc_carousel');
		...
		public function add_theme_options_settings( $options, $section_class = null ) {
			...
			if ( ! empty( $section_class ) && 'pluginsoptions' == $section_class ){
				$options[] = array(  'name'  => __( 'Visual Composer Shortcodes', 'ishyoboy' ),
					'desc'  => __( 'Enable or disable the default Visual Composer shortcodes in IshYoBoy Pagebuilder', 'ishyoboy'),
