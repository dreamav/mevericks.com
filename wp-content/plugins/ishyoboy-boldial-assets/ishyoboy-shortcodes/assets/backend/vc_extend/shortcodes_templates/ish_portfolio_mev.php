...
if ( '' != $sc_atts['filter'] ){
	...
	if ($count > 0) {
		if ( '' != $sc_atts['filter']){
			...
			$filter_otput .= '<li><a class="ish-active" href="#all" data-filter="*">' . __( 'Все', 'ishyoboy' ) . '</a></li>';
		}else{
			$filter_otput .= '<div class="ish-section-filter"><div class="ish-vc_row_inner">';
			if ( '' != $sc_atts['filter_title'] ){
				$filter_otput .= '<div class="ish-sc-element ish-sc_headline ish-p-filter-headline ish-center ish-h3">' . $sc_atts['filter_title'] . '</div>';
...
if ( $wpbp->have_posts() ) {
	...
	$return .= '<div class="carousel-container"><div class="mev_carousel" id="icarousel">';
	...
	while ( $wpbp->have_posts() ) {
		...
		$return .= '<div class="ish-p-col slide';
		...
		if ( ( 'flip' == $sc_atts['animation'] ) || ( '3dcube' == $sc_atts['animation'] ) ){
			...
			$return .= '<div class="ish-p-overlay a-tut-my-zamutim-karusel' . $item_classes . '">';
		}
