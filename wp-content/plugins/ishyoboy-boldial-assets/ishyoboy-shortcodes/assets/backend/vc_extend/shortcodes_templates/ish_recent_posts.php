...
if ( $wpbp->have_posts() ) {
	...
	while ( $wpbp->have_posts() ) {
		...
		if ( 'no' != $sc_atts['show_author'] ) {
			$return .= '<span><span>' . __( 'by', 'ishyoboy') . ' ' . get_the_author() . '</span></span> ';
		}

		if ( 'no' != $sc_atts['show_categories'] ) {
			if ( has_category() ) {
				$return .= '<span><span>' . __( 'in category', 'ishyoboy') . '</span> ';
		...
		if ( 'no' != $sc_atts['show_tags'] ) {
			if ( has_tag() ) {
				$terms = get_the_tags();
				$return .= '<span><span>' . __( 'tagged as', 'ishyoboy') . '</span> ';
		...
		if ('no' != $sc_atts['show_read_more']){
			$return .=  '<a class="ish-recent_posts-read_more ish-sc_button ish-left ' . $color_data['bg_class'] . $color_data['text_class'] . '" href="' . get_permalink() . '">' . __( 'Read more', 'ishyoboy') . '</a>';
		}
		$return .= '</div></div>';
