...
if ( ! function_exists( 'ishyoboy_load_my_scripts' ) ) {
	function ishyoboy_load_my_scripts() {
		...
		wp_register_script( 'ish-icarousel', IYB_HTML_URI_JS . '/vendor/icarousel.min.js', array('jquery'));
		wp_enqueue_script( 'ish-icarousel' );
		// wp_register_script( 'ish-raphael', IYB_HTML_URI_JS . '/vendor/raphael-min.js', array('jquery'));
		// wp_enqueue_script( 'ish-raphael' );
		// wp_register_script( 'ish-mousewheel', IYB_HTML_URI_JS . '/vendor/jquery.mousewheel.js', array('jquery'));
		// wp_enqueue_script( 'ish-mousewheel' );

		// wp_register_script( 'ish-roundabout', IYB_HTML_URI_JS . '/vendor/roundabout.pkgd.min.js', array('jquery'));
		// wp_enqueue_script( 'ish-roundabout' );
		// wp_register_script( 'ish-drag', IYB_HTML_URI_JS . '/vendor/jquery.event.drag-2.2.js', array('jquery'));
		// wp_enqueue_script( 'ish-drag' );
		// wp_register_script( 'ish-drop', IYB_HTML_URI_JS . '/vendor/jquery.event.drop-2.2.js', array('jquery'));
		// wp_enqueue_script( 'ish-drop' );
		...
		wp_register_script( 'ish-custom', IYB_HTML_URI_JS . '/custom.js', array('easing'), null, true );
		wp_enqueue_script( 'ish-main' );
		wp_enqueue_script( 'ish-custom' );
		...
		wp_register_style('mev_style', IYB_HTML_URI_CSS . '/mev_style.css');
		wp_enqueue_style( 'mev_style' );
...
if ( ! function_exists( 'ishyoboy_google_fonts_setup' ) ) {
	function ishyoboy_google_fonts_setup() {
		...
		foreach ($load as $font => $variants){
			$i++;
			wp_enqueue_style( THEME_SLUG . '-google-font-' . $i, "$protocol://fonts.googleapis.com/css?subset=latin,cyrillic&family=" . rawurlencode( $font ) . ':' . rawurlencode( $variants ) );
		}
	}
}
...
require_once( locate_template( 'assets/framework/wp/pagebuilder/ish_config/config.php' ) );