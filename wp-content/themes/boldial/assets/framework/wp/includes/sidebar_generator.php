...
class sidebar_generator {
	    ...
	    if(is_array($sidebars)){
			foreach($sidebars as $sidebar){
				...
				register_sidebar(array(
					'name'=>$sidebar,
			    	'before_widget' => '<div id="%1$s" class="sbg_widget wr1 widget ' . $sidebar_class . ' %2$s">',
		   			'after_widget' => '</div>',
		   			'before_title' => '<h4 class="widget-title sbg_title">',
					'after_title' => '</h4>',
		    	));
			}
		}
	...
	public static function admin_menu(){
		add_theme_page('Sidebars', 'Sidebars', 'manage_options', 'sidebar_generator', array('sidebar_generator','admin_page'));
	}
	...
	public static function admin_page(){
		...
		<div class="wrap">
			<h2>Theme Sidebars</h2>
			<br />
			<div class="add_sidebar">
				<a href="javascript:void(0);" onclick="return add_sidebar_link()" title="Add a sidebar" class="button-secondary">+ Add Sidebar</a>
			</div>
			<br />
			<table class="widefat page" id="sbg_table" style="width:600px;">
				<tr>
					<th>Sidebar Name</th>
					<th>Sidebar CSS class</th>
					<th>Remove</th>
				</tr>
				<?php
				$sidebars = sidebar_generator::get_sidebars();
...
$sbg->init(); // Need to call it right away as other plugins expect the sidebars to be registered sooner
