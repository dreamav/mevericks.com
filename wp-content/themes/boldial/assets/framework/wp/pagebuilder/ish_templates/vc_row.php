...
if ( 'vc_row_inner' == $this->shortcode ){
	...
	if ( !empty( $style ) ){
		str_replace( 'style="', 'style="' . $ish_style . ' ', $style );
	} else{
	...
	$ish_css_classes .= ( '' != $tooltip && '' != $tooltip_color ) ? ( ' ish-tooltip-' . esc_attr( $tooltip_color ) ) : '';
	$ish_css_classes .= ( '' != $tooltip && '' != $tooltip_text_color ) ? ' ish-tooltip-text-' . esc_attr( $tooltip_text_color ) : '';
	$css_class =  apply_filters(VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, 'wpb_row '. get_row_css_class() . ' ' . $full_width_class . $el_class . $ish_css_classes . $c . $tc . $section_class . $bg_svg_class . $top_svg_class . $bottom_svg_class . $row_decor_padding_class, $this->settings['base']);
